...
static const struct lfd_flag sc5xx_lfd_flags[] = {
	{ BFLAG_SAFE, "safe" },		  { BFLAG_AUX, "aux" },
	{ BFLAG_FORWARD, "forward" },	  { BFLAG_FILL, "fill" },
	{ BFLAG_QUICKBOOT, "quickboot" }, { BFLAG_CALLBACK, "callback" },
	{ BFLAG_INIT, "init" },		  { BFLAG_IGNORE, "ignore" },
	{ BFLAG_INDIRECT, "indirect" },	  { BFLAG_FIRST, "first" },
	{ BFLAG_FINAL, "final" },	  { 0, 0 }
};
 ...
 */
void *sc5xx_lfd_read_block_header(LFD *alfd, bool *ignore, bool *fill,
	...
	for (b = 0; b < ldr->dxes[d].num_blocks; ++b) {
		...
		if (quiet) {
			...
			printf("\t\tTarget Address: 0x%08X ( %s )\n",
			       ...
			       (header->target_address > 0x80000000 ? "DDR" :
								      "L2"));
 ...
 */
static bool _sc5xx_lfd_write_header(FILE *fp, uint32_t block_code,
			   ...
			   void *src)
{
	...
	if ((dxe_flags & DXE_BLOCK_FIRST)) {
		...
		if (ftell(fp))
			ret &= sc5xx_lfd_update_first_block(fp,
							    block_code_base);
	...
	if (opts->hole.offset) {
		...
		if (opts->hole.offset > off &&
			...
			if (ssplit_count < LDR_BLOCK_HEADER_LEN) {
				...
				ret &= _sc5xx_lfd_write_header(fp, block_code,
							       addr,
							       split_count,
							       argument);
...
static const char *const sc589_aliases[] = {
	"SC570",  "SC571", "SC572",  "SC573",  "SC582", "SC583",
	"SC584",  "SC587", "SC591",  "SC591W", "SC592", "SC592W",
	"SC593W", "SC594", "SC594W", NULL
};
...
static const char *const sc598_aliases[] = { "SC595",  "SC595W", "SC596",
					     "SC596W", "SC598W", "SC846",
					     NULL };
...
__attribute__((constructor)) static void sc5xx_lfd_target_register(void)
{
