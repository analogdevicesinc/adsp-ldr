...
#define COMMON_HELP_OPTS                                                    \
	{ "Ignore problems", NULL }, { "Make a lot of noise", NULL },       \
		{ "Only show errors", NULL }, { "Enable debugging", NULL }, \
...
#define CASE_common_errors                                       \
	...
	case '?':                                                \
		err("Unknown option '%c' or argument missing",   \
		    (optopt ?: '?'));                            \
	default:                                                 \
...
#define show_usage(status) \
	show_some_usage(argv0, NULL, long_opts, opts_help, PARSE_FLAGS, status)

#define SHOW_PARSE_FLAGS COMMON_FLAGS ""
static struct option const show_long_opts[] = { COMMON_LONG_OPTS };
static struct option_help const show_opts_help[] = { COMMON_HELP_OPTS };
#define show_show_usage(status)                                        \
	show_some_usage(argv0, "show", show_long_opts, show_opts_help, \
			SHOW_PARSE_FLAGS, status)

#define DUMP_PARSE_FLAGS COMMON_FLAGS "F"
static struct option const dump_long_opts[] = { { "fill", no_argument, NULL,
						  'F' },
						COMMON_LONG_OPTS };
...
#define show_dump_usage(status)                                        \
	show_some_usage(argv0, "dump", dump_long_opts, dump_opts_help, \
			DUMP_PARSE_FLAGS, status)
...
#define show_create_usage(status)                                            \
	show_some_usage(argv0, "create", create_long_opts, create_opts_help, \
			CREATE_PARSE_FLAGS, status)
...
#define show_load_usage(status)                                        \
	show_some_usage(argv0, "load", load_long_opts, load_opts_help, \
			LOAD_PARSE_FLAGS, status)
...
static void show_some_usage(const char *argv0, const char *subcommand,
		     ...
		     const char *sirev)
{
	...
	while ((i = getopt_long(argc, argv, SHOW_PARSE_FLAGS, show_long_opts,
				NULL)) != -1) {
		switch (i) {
		case 'h':
			show_show_usage(0);
		     ...
		     const char *sirev)
{
	...
	while ((i = getopt_long(argc, argv, DUMP_PARSE_FLAGS, dump_long_opts,
				NULL)) != -1) {
		switch (i) {
		case 'F':
			opts.dump_fill = true;
		     ...
		     const char *sirev)
{
	...
	while ((i = getopt_long(argc, argv, LOAD_PARSE_FLAGS, load_long_opts,
				NULL)) != -1) {
		switch (i) {
		case 'b':
		       ...
		       const char *sirev)
{
	...
	while ((i = getopt_long(argc, argv, CREATE_PARSE_FLAGS,
				create_long_opts, NULL)) != -1) {
		switch (i) {
		case 0x2:
			opts.bmode = optarg;
			break;
		case 'p':
			opts.port = toupper(optarg[0]);
			break;
		case 'g':
			opts.gpio = atoi(optarg);
			break;
		case 'd':
			opts.dma = atoi(optarg);
			break;
		case 0x5:
			...
			if (opts.bcode > 15)
				err("Invalid bcode '%d' specified.",
				    opts.bcode);
			break;
		case 'B':
			opts.flash_bits = atoi(optarg);
			break;
		case 'w':
			opts.wait_states = atoi(optarg);
			break;
		case 'H':
			opts.flash_holdtimes = atoi(optarg);
			break;
		case 's':
			opts.spi_baud = atoi(optarg);
			break;
		case 'b':
			/* support reading in hex values since it's much more
				 * common for people to set size in terms of hex ...
			...
			if (opts.block_size == 0 || opts.block_size % 4)
				err("Invalid block size '%d' specified.",
				    opts.block_size);
			break;
		case 'i':
			opts.init_code = optarg;
			break;
		case 'P':
			/* support reading in hex values since it's much more
				 * common for people to set size in terms of hex ...
				 */
			if (sscanf(optarg, "%zi:%zi", &opts.hole.offset,
				   &opts.hole.length) != 2)
				if (sscanf(optarg, "%zX:%zX", &opts.hole.offset,
					   &opts.hole.length) != 2)
					err("Unable to parse offset:size from '%s'",
					    optarg);
			...
			if (filler_file) {
				...
				if (filler_file++) {
					if (access(filler_file, R_OK))
						err("Unable to access filler file '%s'",
						    filler_file);
		...
		case 'M':
			...
			break;
		case 'J':
			opts.jump_block = false;
			break;
		case 0x4:
			opts.fill_blocks = false;
			break;
		case 0x3:
			opts.cur_core = 0;
			break;
	...
	if (strchr("?FGH", opts.port) == NULL)
		err("Invalid PORT '%c'.  Valid PORT values are 'F', 'G', and 'H'.",
		    opts.port);
	if (opts.gpio > 16)
		err("Invalid GPIO '%i'.  Valid GPIO values are 0 - 16.",
		    opts.gpio);
	if (opts.dma < 1 || opts.dma > 15)
		err("Invalid DMA '%i'.  Valid DMA values are 1 - 15.",
		    opts.dma);
	if (opts.flash_bits != 8 && opts.flash_bits != 16)
		err("Invalid flash bits '%i'.  Valid bits are '8' and '16'.",
		    opts.flash_bits);
	if (opts.wait_states > 15)
		err("Invalid number of wait states '%i'.  Valid values are 0 - 15.",
		    opts.wait_states);
	if (opts.flash_holdtimes > 3)
		err("Invalid number of flash hold time cycles '%i'.  Valid values are 0 - 3.",
		    opts.flash_holdtimes);
	if (opts.spi_baud != 500 && opts.spi_baud != 1000 &&
	    opts.spi_baud != 2000)
		err("Invalid SPI baud '%i'.  Valid values are 500 (500k), 1000 (1M), or 2000 (2M).",
		    opts.spi_baud);
...
static void prog_failure_signaled(int sig)
{
	...
	if (debug) {
		...
		case 0: {
			...
			printf("\nAuto launching gdb!\n\n");
			ret = execlp("gdb", "gdb", "--quiet", "--pid", pid,
				     "-ex", "bt full", NULL);
			_exit(ret);
		}
		default: {
