				  ...
				  size_t *data_len)
{
	...
	memcpy(&(header->target_address), header->raw + 4,
	       sizeof(header->target_address));
	memcpy(&(header->byte_count), header->raw + 8,
	...
	for (b = 0; b < ldr->dxes[d].num_blocks; ++b) {
		...
		if (quiet) {
			printf("0x%08X 0x%08X 0x%08X 0x%08X ( ",
			       header->block_code, header->target_address,
			       header->byte_count, header->argument);
		} else if (verbose) {
			...
			printf("         Addr: 0x%08X BCode: 0x%08X Bytes: 0x%08X Args: 0x%08X ( ",
			       header->target_address, header->block_code,
			       header->byte_count, header->argument);
		}
		...
		tmp = (header->block_code & BFLAG_HDRSIGN_MASK) >>
		      BFLAG_HDRSIGN_SHIFT;
		 ...
		 */
		tmp = (header->block_code & BFLAG_HDRCHK_MASK) >>
		      BFLAG_HDRCHK_SHIFT;
		...
		case 0:
			...
			break;
		case 1:
			printf("8bit-dma-from-8bit ");
			break;
		case 2:
			printf("8bit-dma-from-16bit ");
			break;
		case 3:
			printf("8bit-dma-from-32bit ");
			break;
		case 4:
			printf("8bit-dma-from-64bit ");
			break;
		case 5:
			printf("8bit-dma-from-128bit ");
			break;
		case 6:
			printf("16bit-dma-from-16bit ");
			break;
		case 7:
			printf("16bit-dma-from-32bit ");
			break;
		case 8:
			printf("16bit-dma-from-64bit ");
			break;
		case 9:
			printf("16bit-dma-from-128bit ");
			break;
		case 10:
			printf("32bit-dma-from-32bit ");
			break;
		case 11:
			printf("32bit-dma-from-64bit ");
			break;
		case 12:
			printf("32bit-dma-from-128bit ");
			break;
		case 13:
			printf("64bit-dma-from-64bit ");
			break;
		case 14:
			printf("64bit-dma-from-128bit ");
			break;
		case 15:
			printf("128bit-dma-from-128bit ");
			break;
		}
...
static bool _bf548_lfd_write_header(FILE *fp, uint32_t block_code,
				    ...
				    uint32_t argument)
{
	...
	if (addr % 4)
		warn("address is not 4 byte aligned (0x%X %% 4 = %i)", addr,
		     addr % 4);
	if (pad_size) {
		warn("count is not 4 byte aligned (0x%X %% 4 = %i)", count,
		     pad_size);
	...
	ldr_make_little_endian_32(block_code);
	block_code |= (compute_hdrchk(header, LDR_BLOCK_HEADER_LEN)
		       << BFLAG_HDRCHK_SHIFT);
	ldr_make_little_endian_32(block_code);
	memcpy(header + 0, &block_code, sizeof(block_code));
	return (fwrite(header, sizeof(uint8_t), LDR_BLOCK_HEADER_LEN, fp) ==
				LDR_BLOCK_HEADER_LEN ?
			true :
			false);
}
...
bool bf54x_lfd_write_block(struct lfd *alfd, uint8_t dxe_flags,
			   const void *void_opts, uint32_t addr, uint32_t count,
			   void *src)
{
	...
	argument = 0xDEADBEEF;
	block_code_base = (BFLAG_HDRSIGN_MAGIC << BFLAG_HDRSIGN_SHIFT) |
	...
	if (opts->hole.offset) {
		...
		if (opts->hole.offset > off &&
			...
			if (ssplit_count < LDR_BLOCK_HEADER_LEN) {
				...
				ret &= _bf548_lfd_write_header(fp, block_code,
							       addr,
							       split_count,
							       argument);
				ret &= (fwrite(src, 1, split_count, fp) ==
							split_count ?
						true :
						false);
				src += split_count;
			...
			ret &= _bf548_lfd_write_header(
				fp, block_code | BFLAG_IGNORE, 0, hole_count,
				0xBAADF00D);
	...
	if (dxe_flags & DXE_BLOCK_FINAL) {
		...
		count = 0;
		argument = 0;
		ret &= _bf548_lfd_write_header(fp, block_code, addr, count,
					       argument);
...
static const char *const bf548_aliases[] = { "BF542",  "BF544",	 "BF547",
					     "BF548",  "BF549",	 "BF542M",
					     "BF544M", "BF547M", "BF548M",
					     "BF549M", NULL };
...
__attribute__((constructor)) static void bf548_lfd_target_register(void)
{
